...
- 作用：查询单物品当前价格。
- 底层调用：`DFPriceService.get_latest_price`（通过私有市场数据后端执行）。
- 输入示例：`非洲之心现在什么价格`
- 输出示例：`非洲之心 的最新价格为 12818829。 更新时间：2026-02-25 07:40:51。`

### 2.3 历史价格查询（`df_market_history_price`）
- 作用：查询历史价格区间统计。
- 底层调用：`DFPriceService.get_history_price`（通过私有市场数据后端执行）。
- 输入示例：`查询一下非洲之心的历史价格`
...
- 作用：返回制造利润 Top1/Top3。
- 底层调用：`DFPriceService.get_place_profit_rank`（通过私有市场数据后端执行）。
...
- 作用：分析某制造品利润“稳不稳”。
- 底层调用：`DFPriceService.get_place_profit_history`（通过私有市场数据后端执行）。
