...
LAUNCH_MODE=${TOOL_SELECTION_SFT_LAUNCH_MODE:-distributed}
TRAIN_CONFIG=${TRAIN_CONFIG:-training/tool_selection_sft/configs/train.json}
ACCEL_CONFIG=${ACCEL_CONFIG:-training/common/configs/accelerate_2gpu_zero2.yaml}

if [[ "$LAUNCH_MODE" == "single" ]]; then
  "$PYTHON_BIN" training/tool_selection_sft/train.py \
