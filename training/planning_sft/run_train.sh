...
LAUNCH_MODE=${PLANNING_SFT_LAUNCH_MODE:-distributed}
TRAIN_CONFIG=${TRAIN_CONFIG:-training/planning_sft/configs/train.json}
ACCEL_CONFIG=${ACCEL_CONFIG:-training/common/configs/accelerate_2gpu_zero2.yaml}

if [[ "$LAUNCH_MODE" == "single" ]]; then
  "$PYTHON_BIN" training/planning_sft/train.py \
