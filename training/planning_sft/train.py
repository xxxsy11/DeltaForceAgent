#!/usr/bin/env python3
"""Planning SFT training entrypoint (Transformers + PEFT + TRL + Accelerate/DeepSpeed)."""
...
class TrainConfig:
    ...
    train_file: str = "data/dataset/final/planning/train.jsonl"
    eval_file: str = "data/dataset/final/planning/dev.jsonl"
    output_dir: str = "outputs/planning_sft/qwen3_8b_lora"
    ...
    run_name: str = "deltaforceagent-planning-sft-lora"
    logging_dir: str = "outputs/planning_sft/logs"
    wandb_project: str = "deltaforceagent-sft"
    wandb_entity: str = ""
    wandb_tags: List[str] = field(default_factory=lambda: ["planning", "sft", "lora", "qwen3-8b"])
...
def normalize_label(label: Dict[str, Any], available_tools: List[str]) -> Dict[str, Any]:
    ...
    for step in task_plan_raw:
        ...
        if tool_name not in available_tools or not tool_query:
            continue
    ...
    return {
        ...
        "reason": str(label.get("reason", "任务规划")).strip() or "任务规划",
        "requires_task_planning": bool(label.get("requires_task_planning", True)),
