...
class IntelligentQueryRouter:
    ...
    def analyze_query(self, query: str) -> QueryAnalysis:
        ...
        # 使用LLM进行智能分析
        analysis_prompt = f"""
        作为RAG系统的查询分析专家，请深度分析以下查询的特征（领域：三角洲行动知识图谱）：
        ...
        1. 查询复杂度 (0-1)：
           - 0.0-0.3: 简单信息查找（如：M4A1是什么？）
           - 0.4-0.7: 中等复杂度（如：哪些房卡属于高物资区？）
           - 0.8-1.0: 高复杂度推理（如：某地图中高等级房卡和可用装备的关联）
        ...
        try:
            ...
            if not result:
                raise ValueError("LLM未返回可解析JSON")

            strategy_raw = str(result.get("recommended_strategy", SearchStrategy.HYBRID_TRADITIONAL.value)).strip()
            ...
            analysis = QueryAnalysis(
                ...
                recommended_strategy=SearchStrategy(strategy_raw),
